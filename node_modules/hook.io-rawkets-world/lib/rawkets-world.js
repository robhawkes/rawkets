...
RawketsWorld.prototype.initSocket = function() {
	...
	self.on("*::socket::disconnect", function(id) {
		...
		self.emit("socket::broadcast", self.encodeMessage([self.typeIndexes.removePlayer, playerId]));
	});
		...
		if (msg) {
			...
			switch (typeIndex) {
				case self.typeIndexes.ping:
					var time = Date.now();
					self.emit("socket::send::"+id, self.encodeMessage([self.typeIndexes.ping, time.toString()]));
				...
				case self.typeIndexes.sync:
					...
					self.emit("socket::broadcast::"+id, self.encodeMessage([self.typeIndexes.newPlayer, localPlayer.id, localPlayer.name, localPlayerState.p.x, localPlayerState.p.y, localPlayerState.a, localPlayerState.h, localPlayerState.f, localPlayer.colour]));
					...
					for (p = 0; p < playerCount; p++) {
						...
						self.emit("socket::send::"+id, self.encodeMessage([self.typeIndexes.newPlayer, player.id, player.name, playerState.p.x, playerState.p.y, playerState.a, playerState.h, playerState.f, player.colour]));
					}
					...
					for (p = 0; p < aiPlayerCount; p++) {
						...
						// Need to queue these messages if the client isn't fully synced up and ready yet
						self.emit("socket::send::"+id, self.encodeMessage([self.typeIndexes.newPlayer, aiPlayer.player.id, aiPlayer.player.name, aiPlayerState.p.x, aiPlayerState.p.y, aiPlayerState.a, aiPlayerState.h, aiPlayerState.f, aiPlayer.player.colour]));
					}
					...
					self.emit("socket::send::"+id, self.encodeMessage([self.typeIndexes.syncCompleted]));
					break;
...
RawketsWorld.prototype.update = function(self) {
	...
	self.bullets.collisionAI(self.aiPlayers, self.msgOutQueue, self.encodeMessage, self.typeIndexes); // AI always die first
	self.bullets.collision(self.players, self.msgOutQueue, self.encodeMessage, self.typeIndexes);
	...
	for (i = 0; i < playerCount; i++) {
		...
		if (player.getInput().fire == 1 && Date.now() - player.bulletTime > 400+Math.round(Math.random()*300)) {
			...
			// Need to cut down the ID
			self.bullets.add("bullet"+Date.now()+player.id, player.id, bulletPos.x, bulletPos.y, player.currentState.a, self.msgOutQueue, self.encodeMessage, self.typeIndexes);
	...
	for (i = 0; i < aiPlayerCount; i++) {
		...
		if (aiPlayer.player.getInput().fire == 1 && Date.now() - aiPlayer.player.bulletTime > 400+Math.round(Math.random()*300)) {
			...
			// Need to cut down the ID
			self.bullets.add("bullet"+Date.now()+aiPlayer.player.id, aiPlayer.player.id, bulletPos.x, bulletPos.y, aiPlayer.player.currentState.a, self.msgOutQueue, self.encodeMessage, self.typeIndexes);
