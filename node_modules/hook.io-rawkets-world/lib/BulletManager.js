...
var BulletManager = function() {
	...
	var add = function(id, playerId, x, y, a) {
	...
	var collision = function(players, self, encodeMessage, typeIndexes) {
		var bullet,
			bulletState,
			bulletCount = bullets.length,
		...
		for (i = 0; i < bulletCount; i++) {
			...
			self.emit("socket::broadcast", encodeMessage([typeIndexes.updateBullet, bullet.id, bulletState.p.x, bulletState.p.y]));
		}
		...
		for (i = bulletCount-1; i >= 0; i--) {
			...
			if (bullet.currentState.h === 0 || Date.now() - bullet.born > 750) {
				// This really needs to be moved into an event that is picked up by the main game logic
				self.emit("socket::broadcast", encodeMessage([typeIndexes.removeBullet, bullet.id]));
	...
	var collisionAI = function(aiPlayers, self, encodeMessage, typeIndexes) {
		var bullet,
			bulletState,
			bulletCount = bullets.length,
