...
var http = require("http"), 
	...
	MESSAGE_TYPE_UPDATE_PLAYER = 4,
	MESSAGE_TYPE_UPDATE_INPUT = 5;
...
socket.on("connection", function(client){
	...
	var newPlayer = Player.init(client.sessionId);
	players.push(newPlayer);
	// Send new player to other clients
	client.broadcast(formatMessage(MESSAGE_TYPE_NEW_PLAYER, {id: newPlayer.id, t: currentTime.toString(), s: newPlayer.getState()}));
	
	console.log("New player added to game: ", client.sessionId);
		...
		if (msg.z !== undefined) {
			switch (msg.z) {
				case MESSAGE_TYPE_PING:
					...
					client.send(formatMessage(MESSAGE_TYPE_PING, {t: time.toString()}));
					break;
				case MESSAGE_TYPE_SYNC:
					var localPlayer = playerById(client.sessionId);
					...
					for (p = 0; p < playerCount; p++) {
						...
						client.send(formatMessage(MESSAGE_TYPE_NEW_PLAYER, {id: player.id, t: currentTime.toString(), s: player.getState()}));
					};
					
					// Sync complete
					// Should this wait until client has confirmed sync is complete?
					client.send(formatMessage(MESSAGE_TYPE_SYNC, {}));
					break;
